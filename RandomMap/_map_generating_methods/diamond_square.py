...
def GenerateNoise(GAME_WIDTH):
    ...
    stepSize = int(w - 1)  # First coordinate is 0,0

    # scale = 2**-1
    scale = 1.0 / w

    # Add init values to the 4 corners
    set_sample(0, 0, scaling(scale, stepSize))
    set_sample(w - 1, 0, scaling(scale, stepSize))
    set_sample(0, w - 1, scaling(scale, stepSize))
    set_sample(w - 1, w - 1, scaling(scale, stepSize))
    ...
    while stepSize >= 2:
        ...
        while x < w - 1:
            y = 0
            while y < w - 1:
                a = get_sample(x, y)  # Top-left corner
                # print('stepSize: ' + str(stepSize))
                b = get_sample(x + stepSize, y)  # Top-right corner
                c = get_sample(x, y + stepSize)  # Bottom-left
                d = get_sample(x + stepSize, y + stepSize)  # Bottom-right

                middle_point = (a + b + c + d) / 4 + scaling(scale, stepSize)

                # print('x: ' + str(x))
                # print('y: ' + str(y))
                # print('halfStep: ' + str(halfStep))

                set_sample(x + halfStep, y + halfStep, middle_point)
        ...
        while x < w - 1:
            ...
            while y < w - 1:
                ...
                c = get_sample(x, y + stepSize)  # Bottom-left
                d = get_sample(x + stepSize, y + stepSize)  # Bottom-right
                e = get_sample(x + halfStep, y + halfStep)

                f = (a + e + c) / 3 + scaling(scale, stepSize)
                set_sample(x, y + halfStep, f)

                g = (a + e + b) / 3 + scaling(scale, stepSize)
                set_sample(x + halfStep, y, g)

                h = (e + b + d) / 3 + scaling(scale, stepSize)
                set_sample(x + halfStep + halfStep, y + halfStep, h)

                i = (c + e + d) / 3 + scaling(scale, stepSize)
                set_sample(x + halfStep, y + halfStep + halfStep, i)
...
def MakeIsland(coordinates, w, shift_y=1.0):
    ...
    while y < w:
        ...
        while x < w:
            ...
            coordinates[i][2] = (
                (
                    coordinates[i][2] + 1.3 - math.sqrt(
                        xd * xd + yd * yd) * 3) * 60 + 120)  # 1.4 is the size of the island
