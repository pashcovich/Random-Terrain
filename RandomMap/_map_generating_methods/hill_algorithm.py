...
def HillAlgorithm(GAME_WIDTH, GAME_HEIGHT):
    ...
    for i in range(ITERATIONS):
        ...
        for coordinate in world_coordinates:
            ...
            if math.sqrt((center_coordinate[0] - coordinate[0]) ** 2 + (
                center_coordinate[1] - coordinate[1]) ** 2) <= radius:
        ...
        for i in range(iterations):
            ...
            for i in range(5):
                ...
                if random.randrange(0, 50) == 0:
                    ...
                    for coordinate in world_coordinates:
                        # Choose all the coordinates within the circle
                        if math.sqrt((moved_coordinate[0] - coordinate[0]) ** 2 + (
                            moved_coordinate[1] - coordinate[1]) ** 2) <= radius:
                            subsequence.append(coordinate)
                ...
                for s_coord in subsequence:
                    ...
                    if previous_section == 1:  # If we add grass
                        ...
                        if water == False:
                            if world_coordinates[index][
                                2] == previous_section:  # Don't raise the height of the land again
                                world_coordinates[index][2] += 1
